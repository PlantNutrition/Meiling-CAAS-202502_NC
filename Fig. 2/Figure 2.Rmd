...
#Statistical test
# rate$Treatment <- factor(rate$Treatment)
# group <- levels(rate$Treatment)
# group1 <- NULL
# group2 <- NULL
# p <- NULL
...
#         group_ij <- subset(rate, Treatment %in% c(group[i], group[j]))
#         group_ij$Treatment <- factor(group_ij$Treatment, levels = c(group[i], group[j]))
#         wilcox_test <- wilcox.test(Copy ~ Treatment, data = group_ij, alternative = 'two.sided', conf.level = 0.95)
